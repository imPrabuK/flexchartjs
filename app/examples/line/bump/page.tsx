...
type OptionLike = {
  ...
  yAxis?: { type?: string; inverse?: boolean; min?: number; max?: number };
  series?: {
    name?: string;
    data?: number[];
    type?: string;
...
function BumpChartPreview({ option }: { option: OptionLike | null }) {
  const { seriesData, xLabels, minY, maxY } = useMemo(() => {
    ...
    const yAxisMin = option.yAxis?.min ?? 1;
    const yAxisMax = option.yAxis?.max ?? 4;
    const inverse = option.yAxis?.inverse ?? false;
    ...
    const seriesPoints = allSeries.map((series, seriesIdx) => {
      ...
      const pts = numericY.map((y, idx) => {
        ...
        const normalized = inverse
          ? (yAxisMax - y) / (yAxisMax - yAxisMin)
          : (y - yAxisMin) / (yAxisMax - yAxisMin);
          ...
          {seriesData.map((series, idx) => (
            <g key={idx}>
              ...
              {series.points.map((pt, ptIdx) => {
                ...
                const rank = series.ranks?.[ptIdx] ?? 0;
                return (
                  <g key={ptIdx}>
                    <circle cx={x} cy={y} r={8} fill={series.color} />
                    <text
                      x={x}
                      y={y + 3}
                      textAnchor="middle"
...
export default function BumpChartExamplePage() {
        ...
        <div className="grid gap-4 md:grid-cols-[minmax(0,1.15fr)_minmax(0,1.35fr)]">
          ...
          <section className="flex flex-col rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="flex-1 rounded-lg bg-slate-50 p-4">
              <BumpChartPreview option={renderedOption} />
            </div>
