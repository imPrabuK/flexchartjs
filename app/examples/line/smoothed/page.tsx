...
type OptionLike = {
  ...
  yAxis?: { type?: string };
  series?: { data?: number[]; type?: string; smooth?: boolean }[];
};
...
function createSmoothPath(points: string[]): string {
  if (points.length < 2) return "";
  if (points.length === 2) return `M ${points[0]} L ${points[1]}`;
  ...
  for (let i = 0; i < points.length - 1; i++) {
    ...
    const cpX = (x1 + x2) / 2;
    const cpY = y1;
    path += ` Q ${cpX},${cpY} ${(x1 + x2) / 2},${(y1 + y2) / 2}`;
    path += ` T ${x2},${y2}`;
  }
  return path;
}

function SmoothedLinePreview({ option }: { option: OptionLike | null }) {
  ...
  const smoothPath = useMemo(() => createSmoothPath(points), [points]);
    ...
    <div className="h-full rounded-md border border-slate-200 bg-white px-4 py-3">
      {!points.length ? (
        ...
        <svg viewBox="0 0 400 260" className="h-full w-full">
          <defs>
            <linearGradient id="smoothArea" x1="0" x2="0" y1="0" y2="1">
          ...
          {points.length > 1 && (
            <path
              d={`${smoothPath} L 368,220 L 32,220 Z`}
              fill="url(#smoothArea)"
              stroke="none"
            />
          )}
...
export default function SmoothedLineExamplePage() {
        ...
        <div className="grid gap-4 md:grid-cols-[minmax(0,1.15fr)_minmax(0,1.35fr)]">
          ...
          <section className="flex flex-col rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="flex-1 rounded-lg bg-slate-50 p-4">
              <SmoothedLinePreview option={renderedOption} />
            </div>
